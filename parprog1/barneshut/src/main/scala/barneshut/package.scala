...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    val massX: Float = bodies.map(b => b.mass * b.x / mass).sum
    val massY: Float = bodies.map(b => b.mass * b.y / mass).sum
    val total: Int = bodies.size
    def insert(b: Body): Quad =
      if (size > minimumSize) {
        (bodies :+ b).foldLeft(
          Fork(
            Empty(centerX - size / 4f, centerY - size / 4f, size / 2f),
            Empty(centerX + size / 4f, centerY - size / 4f, size / 2f),
            Empty(centerX - size / 4f, centerY + size / 4f, size / 2f),
            Empty(centerX + size / 4f, centerY + size / 4f, size / 2f))) { (left, right) =>
          left.insert(right)
        }
      } else {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case f @ Fork(nw, ne, sw, se) =>
          ...
          if (quad.size / distance(x, y, f.massX, f.massY) < theta) {
